...
Renderer::InitialiseOpenGL(int screenWidth, int screenHeight)
{
    ...
    m_pWindow = SDL_CreateWindow("Asteroids", SDL_WINDOWPOS_UNDEFINED, 
...
void 
Renderer::DrawSprite(Sprite& sprite, Vector2& position)
{
    m_pSpriteShader->SetActive();
    ...
    world.m[3][0] = static_cast<float>(position.x);
    world.m[3][1] = static_cast<float>(position.y);
...
void
Renderer::DrawAnimatedSprite(SpriteSheetAnimation& sprite, Vector2& position, int frame)
{
	m_pSpriteShader->SetActive();
	float angleInDegrees = sprite.GetAngle();
...
Renderer::CreateStaticTextSprite(const char* text, FontSize fSize, SDL_Color color, SpriteOrigin origin)
{
	...
	pTexture->LoadTextTexture(text, GetFontFromEnum(fSize), color);
	m_pTextureManager->AddTexture(text, pTexture);
